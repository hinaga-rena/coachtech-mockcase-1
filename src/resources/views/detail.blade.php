...
<div class="container">
  <div class="item">
    @php
      // 商品画像のURL（DBには products/... で保存されている）
      $itemImg = asset($item->img_url);
    @endphp

    @if ($item->sold())
      <div class="item__img sold">
        <img src="{{ $itemImg }}" alt="商品画像">
      </div>
    @else
      <div class="item__img">
        <img src="{{ $itemImg }}" alt="商品画像">
    ...
    <div class="item__info" id="scroll__item__info">
      ...
      @else
        <a href="{{ route('purchase.index', $item->id) }}" class="btn item__purchase">購入手続きへ</a>
      @endif
      ...
      <div id="comment" class="comment_section">
        ...
        <div class="comments" id="comments__list">
          @foreach ($item->getComments() as $comment)
            @php
              $profileImg = $comment->user->profile->img_url
                ? asset($comment->user->profile->img_url)
                : asset('img/icon.png');
            @endphp
            <div class="comment">
              <div class="comment__user">
                <div class="user__img">
                  <img src="{{ $profileImg }}" alt="ユーザーアイコン">
                </div>
